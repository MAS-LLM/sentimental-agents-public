...
class SentimentTracker:
    ...
    def get_last_similarity(self, name):
        """
        Retrieve the last similarity value for a given agent.
        """
        if len(self.similarities[name]) > 0:
            return self.similarities[name][-1]
        return 0.0  # Default to 0.0 if no similarity has been computed
